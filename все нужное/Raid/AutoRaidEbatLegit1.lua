...
local function moveTo(targetPosition)
    ...
    if not hrp or not _G.AutoFarmRaidNormal then return end

    -- Останавливаем текущий твин, если он запущен
    if currentTween then
        currentTween:Cancel()
    end

    local distance = (targetPosition - hrp.Position).Magnitude
    -- Рассчитываем время: время = дистанция / скорость
    ...
    -- Ждем окончания перемещения или остановки скрипта
    ...
    repeat task.wait() until completed or not _G.AutoFarmRaidNormal
end
